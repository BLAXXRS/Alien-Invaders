"""
Alien Invaders — Fixed Edition (with hit-flash + knockback)
...
class BuffDrop:
    ...
    def draw(self, surf):
        ...
        if self.kind == 'multishot':
            ...
            pygame.draw.polygon(surf, RED, [(x, y-5), (x-6,y+2),(x+6,y+2)])
            pygame.draw.circle(surf, RED, (x-3, y-3), 3); pygame.draw.circle(surf, RED, (x+3, y-3), 3)
                ...
                if ev.type == pygame.KEYDOWN:
                    ...
                    if ev.key == pygame.K_RETURN:
                        ...
                        if player.score >= cost:
                            ...
                            elif code == "heal": player.hp = player.hp_max
...
def main():
    ...
    pygame.display.set_caption("Alien Invaders — Fixed")
    ...
    while True:
        ...
        if state == 'playing':
            ...
            for e in wave.enemies:
                if not e.alive: continue
                base_prob = wave.enemy_shoot_prob * 0.6
            ...
            for d in drops[:]:
                if d.rect.colliderect(player_rect):
                    drops.remove(d)
                    if d.kind == 'multishot': player.multishot_active = True; player.multishot_end_time = pygame.time.get_ticks() + BUFF_DURATION_MS
                    elif d.kind == 'shield': player.shield_active = True; player.shield_uses = 3
                    elif d.kind == 'heal': player.hp = min(player.hp_max, player.hp + 2)
            ...
            if in_shop:
                ...
                bullets=[]; enemy_bullets=[]; drops=[]; explosions=[]; player.hp = min(player.hp + 1, player.hp_max)
